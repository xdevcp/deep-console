...
import cc.devcp.project.config.server.component.DataSourceProps;
...
import static cc.devcp.project.config.server.service.PersistService.CONFIG_INFO4BETA_ROW_MAPPER;
    ...
    public synchronized void reload() throws IOException {
        ...
        try {
            ...
            for (int i = 1; i <= dbNum; i++) {
                ...
                if (null == val) {
                    fatalLog.error("spring.datasource.custom-db-url-" + i + " is null");
                ...
                if (null == val) {
                    fatalLog.error("spring.datasource.username-" + i + " is null");
                    throw new IllegalArgumentException();
                }
                ...
                if (null == val) {
                    fatalLog.error("spring.datasource.password-" + i + " is null");
                    throw new IllegalArgumentException();
                }
