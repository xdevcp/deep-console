...
import cc.devcp.project.provider.model.CacheItem;
import cc.devcp.project.provider.model.ConfigInfoBase;
import cc.devcp.project.provider.utils.*;
...
public class ConfigService {
    ...
    static public boolean dump(String dataId, String group, String tenant, String content, long lastModifiedTs, String type) {
        ...
        if (lockResult < 0) {
            LogUtil.dumpLog.warn("[dump-error] write lock failed. {}", groupKey);
            return false;
        }
        ...
        try {
            ...
            } else if (!STANDALONE_MODE || PropertyUtil.isStaticUseMysql()) {
                DiskUtil.saveToDisk(dataId, group, tenant, content);
            }
            updateMd5(groupKey, md5, lastModifiedTs);
            return true;
        } catch (IOException ioe) {
            LogUtil.dumpLog.error("[dump-exception] save disk error. " + groupKey + ", " + ioe.toString(), ioe);
            if (ioe.getMessage() != null) {
                ...
                if (NO_SPACE_CN.equals(errMsg) || NO_SPACE_EN.equals(errMsg) || errMsg.contains(DISK_QUATA_CN)
                    ...
                    // 磁盘写满保护代码
                    LogUtil.fatalLog.error("磁盘满自杀退出", ioe);
                    System.exit(0);
                }
            }
            return false;
    ...
    static public boolean dumpBeta(String dataId, String group, String tenant, String content, long lastModifiedTs,
        ...
        if (lockResult < 0) {
            LogUtil.dumpLog.warn("[dump-beta-error] write lock failed. {}", groupKey);
            return false;
        }
        ...
        try {
            ...
            LogUtil.dumpLog.error("[dump-beta-exception] save disk error. " + groupKey + ", " + ioe.toString(),
                ioe);
            return false;
        } finally {
            releaseWriteLock(groupKey);
    ...
    static public boolean dumpTag(String dataId, String group, String tenant, String tag, String content,
        ...
        if (lockResult < 0) {
            LogUtil.dumpLog.warn("[dump-tag-error] write lock failed. {}", groupKey);
            return false;
        }
        ...
        try {
            ...
            LogUtil.dumpLog.error("[dump-tag-exception] save disk error. " + groupKey + ", " + ioe.toString(),
                ioe);
            return false;
        } finally {
            releaseWriteLock(groupKey);
    ...
    static public boolean dumpChange(String dataId, String group, String tenant, String content, long lastModifiedTs) {
        ...
        try {
            final String md5 = MD5.getInstance().getMD5String(content);
            if (!STANDALONE_MODE || PropertyUtil.isStaticUseMysql()) {
            ...
            LogUtil.dumpLog.error("[dump-exception] save disk error. " + groupKey + ", " + ioe.toString(),
                ioe);
            return false;
        } finally {
            releaseWriteLock(groupKey);
    ...
    static public void reloadConfig() {
        ...
        try {
            ...
            LogUtil.dumpLog.error("reload fail:" + AggrWhitelist.AGGRIDS_METADATA, e);
        }
        ...
        try {
            ...
            LogUtil.dumpLog.error("reload fail:"
        ...
        try {
            ...
            LogUtil.dumpLog.error("reload fail:" + SwitchService.SWITCH_META_DATAID, e);
        }
        ...
        for (Entry<String/* groupKey */, CacheItem> entry : CACHE.entrySet()) {
            ...
            try {
                ...
                if (!entry.getValue().md5.equals(loacalMd5)) {
                    LogUtil.defaultLog.warn("[md5-different] dataId:{},group:{}",
                        dataId, group);
                    diffList.add(groupKey);
                }
            } catch (IOException e) {
                LogUtil.defaultLog.error("getLocalConfigMd5 fail,dataId:{},group:{}",
                    dataId, group);
            }
        }
        long endTime = System.currentTimeMillis();
        LogUtil.defaultLog.warn("checkMd5 cost:{}; diffCount:{}", endTime - startTime,
            diffList.size());
        return diffList;
    }
    ...
    static public boolean remove(String dataId, String group, String tenant) {
         ...
         */
        if (0 == lockResult) {
            LogUtil.dumpLog.info("[remove-ok] {} not exist.", groupKey);
            return true;
        }
        /**
         * 加锁失败
         */
        if (lockResult < 0) {
            LogUtil.dumpLog.warn("[remove-error] write lock failed. {}", groupKey);
            return false;
        }
    ...
    static public int tryReadLock(String groupKey) {
        ...
        if (result < 0) {
            LogUtil.defaultLog.warn("[read-lock] failed, {}, {}", result, groupKey);
        }
        return result;
    }
    ...
    static int tryWriteLock(String groupKey) {
        ...
        if (result < 0) {
            LogUtil.defaultLog.warn("[write-lock] failed, {}, {}", result, groupKey);
        }
        return result;
    }
