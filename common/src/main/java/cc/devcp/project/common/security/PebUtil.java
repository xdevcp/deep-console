...
public class PebUtil {
     ...
     * 默认密钥
     */
    private static final String SECRET_KEY_DEFAULT = "kku+li2nvouVl0FKjzF3HB3IKucspoc9";

    /**
     * 加密算法中使用的盐值,解密中使用的盐值必须与加密中使用的相同才能完成操作. 盐长度必须为8字节
     */
    private static final String SECRET_SALT = "63293188";
    ...
    public static String encrypt(String text) {
        return encrypt(text, SECRET_KEY_DEFAULT, PebUtil.getStaticSalt());
    }

    public static String encrypt(String text, String secretKey) {
        return encrypt(text, secretKey, PebUtil.getStaticSalt());
    }
    ...
    public static String decrypt(String encryptText) {
        return decrypt(encryptText, SECRET_KEY_DEFAULT, PebUtil.getStaticSalt());
    }

    public static String decrypt(String encryptText, String secretKey) {
        return decrypt(encryptText, secretKey, PebUtil.getStaticSalt());
    }
    ...
    public static String decrypt(String ciphertext, String password, byte[] salt) {
        ...
        PBEParameterSpec parameterSpec = new PBEParameterSpec(
                PebUtil.getStaticSalt(), ITERATION_COUNT);
        try {
            Cipher cipher = Cipher.getInstance(ALGORITHM);
